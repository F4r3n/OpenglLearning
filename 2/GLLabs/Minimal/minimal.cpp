...
void makeAColoredTorus(ensi::gl::Mesh& torus, double R, double r, int nbBins)/*{{{*/
{
	...
	for(double theta = 0; theta < 2*glm::pi<float>(); theta+=2*glm::pi<float>() / nbBins) {
		...
		for(double phi = 0; phi <2* glm::pi<float>() +glm::pi<float>() / nbBins; phi += 2*glm::pi<float>() / nbBins) {
			...
			Vertex v(p,torusNormal(theta,phi),glm::vec2(p[0],p[1]),glm::vec4(0.5,0.5,0,1));
			torus.m_verts.push_back(v);


			torus.m_tris.push_back(indexT*nbBins+indexP);
			torus.m_tris.push_back(indexT*nbBins+(indexP+1)%nbBins);
			torus.m_tris.push_back(((indexT+1)%nbBins)*nbBins+indexP);

			torus.m_tris.push_back(((indexT+1)%nbBins)*nbBins+indexP);
			torus.m_tris.push_back(indexT*nbBins+(indexP+1)%nbBins);
			torus.m_tris.push_back(((indexT+1)%nbBins)*nbBins+(indexP+1)%nbBins);
